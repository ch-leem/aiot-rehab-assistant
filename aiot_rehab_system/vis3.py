...
def main():
    ...
    ap.add_argument("--imu_ymax", type=float, default=300.0)
    ap.add_argument("--vis_ymin", type=float, default=0.0)
    ap.add_argument("--vis_ymax", type=float, default=300.0)

    # vis2만 EMA 로우패스 적용
    ap.add_argument("--vis2_ema_hz", type=float, default=12.0)
    ...
    # scaling for playback to target_fps if using timestamps
    dt_src = []
    ...
    vis_t = deque()
    vis_w = deque()  # 여기에는 smooth 값만 넣는다
    ...
    def ema_update(prev, x, hz, dt_sec):
        if prev is None or not math.isfinite(prev):
            return x
        k = 1.0 - math.exp(-hz * dt_sec)
    ...
    try:
        while True:
            ...
            # vis2 raw 계산
            w_vis_raw, last_angle_state = elbow_angvel_degps(r, ts, last_angle_state)

            # vis2 EMA 업데이트, dt는 timestamp 기반
            if len(vis_t) > 0:
                dt_vis = max(1e-6, ts - vis_t[-1])
            else:
                dt_vis = 1.0 / max(1e-6, args.target_fps)

            if math.isfinite(w_vis_raw):
                vis2_state = ema_update(vis2_state, w_vis_raw, args.vis2_ema_hz, dt_vis)

            w_vis_s = vis2_state if (vis2_state is not None) else math.nan

            push_window(imu_t, imu_v, ts, imu_val, args.history_sec)
            push_window(vis_t, vis_w, ts, w_vis_s, args.history_sec)
            ...
            plot_timeseries(
                ...
                list(vis_t),
                list(vis_w),
                title=f"Vision angular velocity (smoothed EMA {args.vis2_ema_hz:.1f}Hz)",
                y_label="deg/s",
            ...
            cv2.putText(
                canvas,
                f"frame {cur_frame}   imu {cur_imu:.3f} cm/s   vis2_smooth {cur_w:.3f} deg/s   conf_th {args.conf_th:.2f}   "
