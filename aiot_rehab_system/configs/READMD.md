...
메인 실행은 `pose_sensor_fusion/run.yaml`을 사용하며, 기본 템플릿은 `default.yaml`입니다.
...
- `pose_sensor_fusion/`
  ...
  - `visualize_replay.yaml`: NDJSON/CSV 재생 시각화 설정

- `data_format_exam/`
  - `data_payload.json`: 페이로드 JSON 템플릿
  - `deg_name_mapping.json`: 각도 이름 매핑(참고용)

---

## pose_sensor_fusion 설정 요약

`pose_sensor_fusion/default.yaml` 주요 항목:
...
`data_format_exam/data_payload.json`은 페이로드 프레임 구조의 스키마 예시입니다.
`pose_sensor_fusion/utils/create_payload.py`에서 이 템플릿을 deep copy하여
...
`pose_sensor_fusion/visualize_replay.yaml`은 NDJSON/CSV 재생 시각화 옵션을 제공합니다.
주요 항목:
